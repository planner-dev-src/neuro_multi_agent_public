...
def _sample_platform_report() -> dict[str, object]:
    return {
...
def test_build_market_report_bundle_creates_markdown_json_and_csv(
    tmp_path: Path,
    monkeypatch,
) -> None:
    monkeypatch.setattr(
    ...
    assert set(bundle.keys()) == {"markdown", "json", "csv"}
...
def test_build_market_report_bundle_writes_expected_json_structure(
    tmp_path: Path,
    monkeypatch,
) -> None:
    monkeypatch.setattr(
    ...
    source_meta = platform["source_meta"]
    assert source_meta["pages_total"] == 2
    assert source_meta["pages_ok"] == 2
    assert source_meta["pages_error"] == 0
    assert source_meta["crawler_access_mode"] == "offline_snapshot"
    assert source_meta["snapshot_hits"] == 2
    assert len(source_meta["page_results"]) == 2


def test_build_market_report_bundle_writes_expected_markdown_content(
    tmp_path: Path,
    monkeypatch,
) -> None:
    monkeypatch.setattr(
    ...
    assert "offline_snapshot" in markdown_text


def test_build_market_report_bundle_writes_expected_csv_rows(
    tmp_path: Path,
    monkeypatch,
) -> None:
    monkeypatch.setattr(
...
def test_build_market_report_bundle_handles_multiple_platforms(
    tmp_path: Path,
    monkeypatch,
) -> None:
    monkeypatch.setattr(
    ...
    assert rows[1]["fit_for_learning"] == "medium"
    assert rows[1]["business_model"] == "b2b"