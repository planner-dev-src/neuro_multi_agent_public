...
def test_detect_topics_from_text_matches_multiple_taxonomy_topics():
    text = """
...
def test_classify_catalog_item_uses_item_type_when_supported():
    item = _catalog_item(
        item_id="c1",
        item_type="course",
        title="Anything",
...
def test_classify_catalog_item_falls_back_to_text_markers():
...
def test_normalize_catalog_items_splits_courses_and_programs():
    items = [
        _catalog_item(
            item_id="course-1",
            item_type="course",
...
def test_compute_coverage_metrics_counts_taxonomy_coverage():
    courses = [
        _course(
            course_id="c1",
...
def test_compute_concentration_metrics_returns_expected_distribution_stats():
    courses = [
...
def test_build_platform_analytics_aggregates_breadth_depth_coverage_and_concentration():
    courses = [
        _course(
            course_id="c1",
            topic_primary="llm_agents",
...
def test_build_platform_analytics_from_items_normalizes_and_aggregates():
    items = [
        _catalog_item(
            item_id="course-1",
            item_type="course",
    ...
    assert analytics["platform_name"] == "Example Platform"
    assert analytics["normalized_entities"]["catalog_items_total"] == 3
    assert analytics["normalized_entities"]["courses_total"] == 2
    assert analytics["normalized_entities"]["programs_total"] == 1
